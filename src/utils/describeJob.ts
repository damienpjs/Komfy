...
/** Follows connections up to a node carrying a text input (prompt). */
function resolveText(graph: PromptGraph, conn: unknown, depth = 0): string | undefined {
  ...
  for (const value of Object.values(node.inputs)) {
    const found = resolveText(graph, value, depth + 1);
...
export function describeJob(graph: PromptGraph): JobSummary {
  ...
  if (sampler) {
    ...
    summary.positive = resolveText(graph, sampler.inputs.positive);
    ...
    if (!(isConn(neg) && graph[neg[0]]?.class_type === 'ConditioningZeroOut')) {
      summary.negative = resolveText(graph, neg);
    }
