...
export interface ComfyClient {
   ...
   * Files also carry `size` (bytes) and `ctime` (creation date, epoch s) —
   * optional: an older extension does not emit them yet.
   */
  listFiles(): Promise<
    { path: string; mtime: number; size?: number; ctime?: number; dir?: boolean }[]
  >;
  /**
   * "Deletes" files/folders (moved to <root>/.komfy-trash by the
   * komfy-listing extension — recoverable from the server). Paths are prefixed
