 ...
 * numbered left to right). At patch time, the `persons` field inserts the
 * replacement passes, in one of two modes:
 *  - one identity per face: for each character, the pass
 ...
 *  - same identity for every face: a single DetailerForEach fed the raw
 *    SEGS, which it iterates over natively (however many faces there are).
...
const graph: PromptGraph = {
  ...
  '8': {
    ...
    inputs: {
      images: ['4', 0],
      filename_prefix: 'komfy/krea2-faceswap',
    },
    _meta: { title: 'Save Image' },
  },
};

export const krea2FaceSwap: WorkflowManifest = {
  id: 'krea2-faceswap',
