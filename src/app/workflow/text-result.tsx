...
export default function TextResultScreen() {
  ...
  // Bank the text as soon as it lands. The launch already filed this job as
  // pending, so this normally just resolves it; `record` upserts, which also
  // covers a job that somehow never got filed. Idempotent per promptId, so
  // the poll's re-renders write once.
  useEffect(() => {
    if (!text || !promptId || !workflowId || !nodeId) return;
    useGeneratedPrompts
      .getState()
      .record({ text, workflowId, promptId, nodeId });
  }, [text, promptId, workflowId, nodeId]);
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {!!text && (
        <>
          ...
          {/* This prompt is already banked — the library is where it (and
              every earlier one) can be found again. */}
          <Pressable
            style={({ pressed }) => [
              styles.secondaryBtn,
              pressed && { backgroundColor: colors.surfacePressed },
          ...
          >
            ...
            <Text style={styles.secondaryText}>{t('textResult.library')}</Text>
          </Pressable>
        </>
      )}
    </ScrollView>
